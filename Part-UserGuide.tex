...
\begin{tabular}{|l|l|l|}
 ...
 \hline
 gatekeeper.nntb.no (Gatekeeper) & ola                                                               & some-secret-password \\ \hline
 plc.simula.nornet  (PLC)        & \href{mailto:ola.nordmann@example.com}{ola.nordmann@example.com}  & some-secret-password \\ \hline 
\end{tabular}
\end{center}
...
\begin{itemize}
 \item Gatekeeper Account providing access to a Linux machine that is connected to \noun{NorNet Core} and the outside Internet.
 \item PLC Account providing access to the PLC (see Subsection~\ref{sub:The-Testbed-Management}). Note, that the PLC user name \emph{must} be a valid e-mail address of the user.
\end{itemize}
Possibly, further accounts will be provided, e.g.\ for web server, Wiki servers, etc.. Such accounts are optional and only provided when needed.
Table~\ref{tbl:An-Example-Accounts-Table} provides an example for the user Ola Nordmann.
...
An SSH remote shell connection to the Gatekeeper server\index{Gatekeeper!Secure Shell} can be established e.g.\ by the \texttt{ssh}\index{\texttt{ssh}} command:
\begin{lstlisting}
ssh ola@gatekeeper.nntb.no
\end{lstlisting}
for user ``ola''. For login, the password is required. Key-based authentication will be explained in Subsubsection~\ref{subsub:Key-Based-Authentication}.
...
Secure Copy\nomenclature{SCP}{Secure Copy}\index{Secure Copy}~(SCP\index{SCP|see{Secure Copy}}) uses SSH to transfer files between machines, i.e.\ it is possible to copy files from the local machine to the Gatekeeper\index{Gatekeeper!Secure Copy} and vice versa. The corresponding command is named \texttt{scp}\index{\texttt{scp}}.

To copy a file \texttt{test.data} from the local machine to the Gatekeeper server, use for example:
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.no:
\end{lstlisting}
Note the colon (i.e.\ ``:''!). Without this colon, ssh will behave like a local file copy (i.e.\ \texttt{cp}) and just create a local copy of \texttt{test.data} with name \text{ola@gatekeeper.nntb.no}. SCP will store the file in the home directory (the default place) of the Gatekeeper server. A different directory can be specified after the colon, e.g.\ \texttt{directory/subdirectory} (note, that these directories must exist):
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.no:directory/subdirectory/
\end{lstlisting}
It is also possible to provide a file name for the destination (e.g.\ \texttt{my-first-test.data}):
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.no:directory/my-first_test.data
\end{lstlisting}
SCP can also copy complete directories, using the ``-r'' option for recursive copy:
\begin{lstlisting}
ssh -r test-directory ola@gatekeeper.nntb.no:
\end{lstlisting}
This will copy the whole directory \texttt{test-directory} to the Gatekeeper.

Similarly, the reverse direction -- i.e.\ from the Gatekeeper to the local machine -- can be used:
\begin{lstlisting}
ssh ola@gatekeeper.nntb.no:test.data .
ssh ola@gatekeeper.nntb.no:test.data some-local-directory/
ssh -r ola@gatekeeper.nntb.no:my-test-directory/ .
\end{lstlisting}
Of course, copying from a remote to another remote machine would work as well.


% ---------------------------------------------------------------------------
\subsubsection{Using Remote Synchronisation}
% ---------------------------------------------------------------------------

SCP only copies files and directories. When working with \noun{NorNet Core} experiment results, it becomes handy to synchronise files and directories, i.e.\ only copy data when it has actually been updated. This functionality is provided by the texttt{rsync}\index{\texttt{rsync}} command\index{Gatekeeper!Remote Synchronisation}.

To synchronise the directory \texttt{experiment1/results/} from the Gatekeeper to the local machine use for example:
\begin{lstlisting}
...
\begin{itemize}
 ...
 \item ``-z'': use compression for improve transfer speed over slow connections. However, it comes of course at the cost of increased CPU utilisation.
 \item ``--partial'': allow to resume file transfer after interruption. For example, having transferred 99\% of 1~GiB just needs the transfer the remaining 1\% after aborting and restarting \texttt{rsync}. Otherwise, the transfer of this file will restart from scratch.
 \item ``--progress': shows the progress of the transfer.
 \item ``--delete'': deletes files that are not existing any more in the source directory. Warning: this option can be disastrous when providing the wrong destination (e.g.\ the user's home directory). Use this option with care!
\end{itemize}
...
Instead of typing the password each time for login, the user can also create a SSH public/private key pair on the local machine (i.e.\ the machine used to access the Gatekeeper server). By storing the public key in \texttt{.ssh/authorized\_keys} on the Gatekeeper server, the user can use his private key to authenticate to the Gatekeeper instead of using the password. This is particularly useful when using script-based login.

To create a public/private key pair, the command \texttt{ssh-keygen}\index{\texttt{ssh-keygen}}\footnote{See also \href{man:ssh-keygen}{\texttt{man ssh-keygen}}.} is used:
\begin{lstlisting}
ssh-keygen -P "" -f ~/.ssh/gatekeeper
\end{lstlisting}
The following options are of interest:
\begin{itemize}
 \item ``-P'': specifies the passphrase for the private key. "" sets no passphrase; this is necessary to use the key later in scripts.
 \item ``-f'': provides the output file prefix. In this case, the private key will be stored in \texttt{~/.ssh/gatekeeper} and the corresponding public key in \texttt{~/.ssh/gatekeeper.pub}.
\end{itemize}

The public key (in this example: \texttt{~/.ssh/gatekeeper.pub}) is just a text file with one long line. It can be displayed e.g.\ with:
\begin{lstlisting}
cat ~/.ssh/gatekeeper.pub
\end{lstlisting}
It should be something like ``ssh-rsa <characters and numbers> <user@host>''.

To use this public key for authentication, log into the Gatekeeper and append the key line to the file \texttt{~/.ssh/authorized\_keys}. This can e.g.\ by done by an arbitrary text editor that is available on the Gatekeeper, e.g.\
\texttt{emacs}\index{\texttt{emacs}},
\texttt{joe}\index{\texttt{joe}},
\texttt{nano}\index{\texttt{nano}} or
\texttt{vi}\index{\texttt{vi}}. If the file \texttt{~/.ssh/authorized\_keys}, it can just be created. After creating the file, it is necessary to check the access permissions:
\begin{lstlisting}
chmod 600 ~/.ssh/authorized_keys
\end{lstlisting}
This ensures that only the owner (and root, of course) can read or modify it. 
...
\begin{itemize}
 \item An SSH connection is established to the Gatekeeper server (see Subsubsection~\ref{subsub:Gatekeeper-Using-SSH}).
 
 \item TCP port~2000 of the local machine (i.e.\ the machine running this command) is forwarded via the Gatekeeper to port~443 of the PLC \href{https://plc.simula.nornet}{plc.simula.nornet}\index{plc.simula.nornet}. Note, that \href{https://plc.simula.nornet}{plc.simula.nornet}\index{plc.simula.nornet} is the DNS name of the PLC. Since the Gatekeeper uses the \noun{NorNet Core} DNS service, it can resolve this \noun{NorNet}-internal name (see Subsubsection~\ref{subsub:Tunnelbox-Name-Resolution}).
