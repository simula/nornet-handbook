...
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.so:
\end{lstlisting}
Note the colon (i.e.\ ``:''!). Without this colon, ssh will behave like a local file copy (i.e.\ \texttt{cp}) and just create a local copy of \texttt{test.data} with name \text{ola@gatekeeper.nntb.so}. SCP will store the file in the home directory (the default place) of the gatekeeper server. A different directory can be specified after the colon, e.g.\ \texttt{directory/subdirectory} (note, that these directories must exist):
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.so:directory/subdirectory/
\end{lstlisting}
It is also possible to provide a file name for the destination (e.g.\ \texttt{my-first-test.data}):
\begin{lstlisting}
ssh test.data ola@gatekeeper.nntb.so:directory/my-first_test.data
\end{lstlisting}
SCP can also copy complete directories, using the ``-r'' option for recursive copy:
\begin{lstlisting}
ssh -r test-directory ola@gatekeeper.nntb.so:
\end{lstlisting}
...
\begin{lstlisting}
ssh ola@gatekeeper.nntb.so:test.data .
ssh ola@gatekeeper.nntb.so:test.data some-local-directory/
ssh -r ola@gatekeeper.nntb.so:my-test-directory/ .
\end{lstlisting}
...
rsync -a -v -z --partial --progress --delete \
 ola@gatekeeper.nntb.so:experiment1/results/ experiment1/results/
\end{lstlisting}
...
rsync -a -v -z --partial --progress experiment1/ \
 ola@gatekeeper.nntb.so:experiment1/
\end{lstlisting}
